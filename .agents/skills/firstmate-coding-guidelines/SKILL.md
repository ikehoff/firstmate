...
- A check that cannot run on this machine must report `skip "<reason>"` from `tests/lib.sh`, never `pass`; reporting a skip as a pass lets a green run claim coverage it never had, and `bin/fm-test-run.sh` counts and names real skips instead.
