...
Verified is not the same as installed: the adapter list is knowledge, not an inventory of this machine.
`bin/fm-spawn.sh` resolves the selected harness executable and refuses with `error: <harness> executable not found on PATH` before creating any endpoint, worktree, or task metadata, so that refusal means the runtime is missing here rather than unverified.
Treat it as a blocker under `AGENTS.md` section 4: install that runtime or dispatch on a different verified runtime that is present, and never retry the same spawn expecting a different result.
Session start front-loads the same fact as a `HARNESS_MISSING:` bootstrap line for the resolved crewmate and secondmate runtimes, so the blocker is known before the first dispatch rather than at it; both that line and the spawn refusal ask `bin/fm-harness.sh executable-path`, the one owner of where a runtime lives.
