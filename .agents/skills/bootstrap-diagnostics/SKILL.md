...
description: >-
  ...
  Use whenever the session-start digest's bootstrap section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, BACKEND_INVALID, HARNESS_MISSING, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, FLEET_SYNC, PR_CHECK_MIGRATION, SECONDMATE_SYNC, SECONDMATE_LIVENESS, NUDGE_SECONDMATES, or FMX - or when a standalone bin/fm-bootstrap.sh run prints one of those lines.
...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `shellcheck`, this also covers an installed build whose version is not the pin `bin/fm-lint.sh --required-version` prints, because that gate refuses both cases.
  It blocks validation of firstmate's own shared tracked material only, so it need not delay work on other projects, but do not dispatch a firstmate-repo task on the no-mistakes path until it is resolved: the failure would otherwise land mid-pipeline, and a green test run does not prove the pin is satisfied because the suite's lint assertions skip when `shellcheck` is absent.
  Its install command is the repo's own checksum-verified installer rather than a package manager, so a package-manager `shellcheck` of a different version does not satisfy it.
- `MISSING_MANUAL: <tool> (instructions: <url>)` - tell the captain why the tool is required and give them the printed instructions URL, but do not pass the tool to `bin/fm-bootstrap.sh install`; wait for the captain to complete the manual installation, then rerun session start to confirm the dependency is present.
  For `google-chrome`, the missing piece is the browser itself rather than `chrome-devtools-axi`, which is only the driver; it is manual because a system browser install needs elevated privileges and the captain's consent.
  It blocks agent-side browser work only - screenshots, snapshots, `eval`, and Lighthouse - so it need not delay other projects, but do not accept browser output as evidence until it is resolved: `chrome-devtools-axi open` prints a `page:` block with `refs: 0` before the executable error, which reads like an empty page rather than broken tooling.
  If the captain instead reaches a browser this machine does not launch, the alternative in the printed instructions is `CHROME_DEVTOOLS_AXI_BROWSER_URL`, and the line clears without any local install.
- `BACKEND_INVALID: <name> (known: <names>)` - the resolved runtime backend has no verified dependency or lifecycle contract, so do not dispatch work until the invalid `FM_BACKEND` or `config/backend` value is corrected to one of the listed backends.
- `HARNESS_MISSING: <harness> (role: crewmate|secondmate; <remediation>)` - the named worker runtime is a verified adapter, but its executable is not installed on this machine, so every dispatch on that role would be refused by the spawn preflight before any work started.
  This is not an installable bootstrap tool: never pass a harness to `bin/fm-bootstrap.sh install`.
  Either ask the captain to install that runtime, or dispatch on a different verified runtime that is present, using the per-task harness override rather than silently rewriting the captain's configured default.
  The crewmate and secondmate roles resolve independently, so one role can be reported while the other is fine.
