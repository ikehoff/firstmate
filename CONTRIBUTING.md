...
- Helper scripts in `bin/` are plain bash.
  ...
  It pins one exact shellcheck version and refuses to run under any other; print it with `bin/fm-lint.sh --required-version` and install that build with `bin/fm-install-shellcheck.sh <dir>`, which verifies the download's checksum and is the same installer CI uses.
  Session-start bootstrap reports `MISSING: shellcheck` when that build is absent or a different version is on `PATH`, so the gap surfaces before a change reaches the gate rather than mid-pipeline; note that a green test run does not prove the pin is satisfied, because the suite's own lint assertions skip when shellcheck is absent.
