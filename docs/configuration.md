...
Naming a verified adapter does not mean its runtime is installed here, so session start reports a resolved crewmate or secondmate harness whose executable is absent as a `HARNESS_MISSING:` line and `fm-spawn.sh` refuses that spawn; both ask `fm-harness.sh executable-path <harness>`, which prints the resolved absolute executable, exits 1 when the verified runtime is not installed, and exits 2 for a name that is not a verified adapter.
...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.31.2 or newer, gh-axi, chrome-devtools-axi, lavish-axi, compatible tasks-axi per "Backlog backend" above, quota-axi, and the pinned shellcheck build described below.
...
`shellcheck` is required at one exact version, the pin `bin/fm-lint.sh --required-version` prints, because that script is the repo's own lint command in `.no-mistakes.yaml` and refuses to run under any other build.
Bootstrap therefore reports `MISSING: shellcheck` both when it is absent and when it is installed at a different version, and the printed install command is the repo's checksum-verified `bin/fm-install-shellcheck.sh`, the same one CI runs.
This requirement bites only on work that changes firstmate's own shared tracked material, and it would otherwise surface mid-pipeline rather than at session start; a green test run is not evidence the pin is satisfied, because the suite's own lint assertions skip when `shellcheck` is absent.
A Google Chrome browser is required alongside `chrome-devtools-axi`, because that package is only the driver and the browser it drives is a separate system install outside npm's reach.
Bootstrap therefore reports `MISSING_MANUAL: google-chrome` when no Chrome executable can be found, and it is manual rather than automatic because installing a system browser needs elevated privileges and the captain's consent.
Without one, every visual or browser task fails and fails misleadingly: `chrome-devtools-axi open` prints a `page:` block with `refs: 0` before the executable error, so skimmed output reads like a successful navigation to an empty page rather than broken tooling.
The check stays silent when `CHROME_DEVTOOLS_AXI_BROWSER_URL` or `CHROME_DEVTOOLS_AXI_AUTO_CONNECT` points the driver at a browser this machine need not own, such as a Windows Chrome reached from WSL.
Detection is a presence heuristic over the driver's own probe locations widened with the `PATH` names a package install provides, and it errs toward silence; `FM_BROWSER_PATHS_OVERRIDE` takes a colon-separated list that replaces that probe entirely for a Chrome installed somewhere neither knows.
