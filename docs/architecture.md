...
That owner also defines the whitespace-padding class (`FM_COMPOSER_WS`) and trims it off its own inputs inside `fm_composer_classify_content` before deciding `empty`, so an adapter never needs a local trim upstream of the classifier and must not re-roll one: a per-adapter copy of that trim is the drift this single owner exists to prevent.
