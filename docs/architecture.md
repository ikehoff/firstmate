...
That owner also defines the whitespace-padding class (`FM_COMPOSER_WS`) and trims it off its own inputs inside `fm_composer_classify_content` before deciding `empty`, so no adapter has to trim the content it hands over.
An adapter that must handle padding earlier anyway, for structural row recognition or for tmux's box width signatures, calls the owner's `fm_composer_trim_ws_var` or `fm_composer_pad_to_space_var` rather than re-rolling the idiom: a per-adapter copy of that trim is the drift this single owner exists to prevent.
How a harness pads an otherwise-empty composer row is an unguaranteed rendering detail that changes between harness versions, so the detector is written against the class rather than any one byte sequence, and [Composer padding rendering](verification/supervision.md#composer-padding-rendering) records the current empirical basis.
