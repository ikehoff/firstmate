...
That owner also defines the whitespace-padding class (`FM_COMPOSER_WS`) that an adapter must trim before a row can read `empty`, because how a harness pads an otherwise-empty composer row is an unguaranteed rendering detail that changes between harness versions; the detector is written against the class rather than any one byte sequence, and [Composer padding rendering](verification/supervision.md#composer-padding-rendering) records the current empirical basis.
